...
class plgEditorCKeditor extends JPlugin
{
	...
	function onDisplay( $name, $content, $width, $height, $col, $row, $buttons = true )
	{
		...
		$db =& JFactory::getDBO();
		$db->setQuery('SELECT template FROM #__templates_menu WHERE client_id='.$app->getClientId());
		...
		$pluginsPath ='';
		$pluginsName = array();
		foreach (glob(dirname(__FILE__)."/ckeditor/plugins/*",GLOB_ONLYDIR) AS $dir)
		{
		...
		if($this->params->get( 'ckfinder','1') == 1)
		{
			...
			if(is_array($access))
			{
				if (array_search($gid,$access) !== false)
				{
					$access_true = true;
				}
			}
			...
			if (file_exists(JPATH_BASE.DS."..".DS."plugins".DS."editors".DS."ckeditor".DS."ckfinder".DS.'ckfinder'.DS."ckfinder.php"))
			{
				...
				$saveDir = str_replace(array('$id','$username','$usertype','$group'),array($user->id,$user->username,$user->usertype,$user->gid),$saveDir);
