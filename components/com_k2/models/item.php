...
class K2ModelItem extends JModel
{
	...
	function prepareItem(&$item, $view, $task){
		...
		//Tags
		if(
			($view=='item' && $item->params->get('itemTags')) ||
	...
	function prepareFeedItem(&$item){
		...
		if ($params->get('feedText')=='1'){
			...
			$item->description=$item->introtext.'<br/>'.$item->fulltext;
		...
		if ($params->get('feedTextWordLimit')){
			$item->description=K2HelperUtilities::wordLimit($description,$params->get('feedTextWordLimit'));
		}
